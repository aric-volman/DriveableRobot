...
public class DriveTrain extends SubsystemBase 
{
  ...
  private final WPI_TalonSRX mechanismRoller;

  // Creates a 
  private AHRS navx = new AHRS(SPI.Port.kMXP);

  private ShuffleboardTab DTTab = Shuffleboard.getTab("DriveTrain");
  private GenericEntry DTDisplacement = DTTab.add("Displacement", 0.0).getEntry();
  private GenericEntry LeftVelocity = DTTab.add("Left Native Velocity", 0.0).getEntry();
  private GenericEntry RightVelocity = DTTab.add("Right Native Velocity", 0.0).getEntry();
  ...
  public DriveTrain() 
  {
    ...
    mechanismRoller = new WPI_TalonSRX(1);
  ...
  public void periodic() {
    ...
    tankDrive(RobotContainer.getJoy1().getThrottle()*-0.35, RobotContainer.getJoy1().getY()*-0.35);

    if (RobotContainer.getJoy1().getRawButton(7)) {
      mechanismRoller.set(ControlMode.PercentOutput, -0.5);
    } else if (RobotContainer.getJoy1().getRawButton(8)) {
      mechanismRoller.set(ControlMode.PercentOutput, 0.5);
    } else {
      mechanismRoller.set(ControlMode.PercentOutput, 0.0);
    }
  }
}